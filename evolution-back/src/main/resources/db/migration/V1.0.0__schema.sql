create table game(uuid varchar(36) not null, created timestamp not null default CURRENT_TIMESTAMP(), content text not null, quickplay boolean  not null, traits text, terminated boolean  not null, primary key (uuid));
